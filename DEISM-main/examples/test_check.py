...
def balloon_plot_with_slider(data_dir, sh_order=4, initial_freq=500, initial_r0_rec=0.5):
    """
    ...
    dirs = np.stack([az_m.ravel(), el_m.ravel()], axis=1) # [phi, theta] each row represents point on the sphere, (res)*(2*res+1) points in total 
    ...
    hn_cache = {} 
    kr_values = [k_all[i] * r for i in range(len(freqs))
                 for r in np.arange(r0, 2.0 + 0.001, 0.1)]  # range of rec_r0 : r0 to 2.0, step 0.1
    ...
    for freq_idx in range(len(freqs)):
        full_Pnm_cache[(freq_idx, sh_order)] = SHCs_from_pressure_LS(Psh[freq_idx].reshape(1, -1), Dir_all, sh_order, np.array([freqs[freq_idx]]))
        k = k_all[freq_idx]
        Cnm_s_cache[(freq_idx, sh_order, r0)] = get_directivity_coefs(k, sh_order, full_Pnm_cache[(freq_idx, sh_order)], r0)
    ...
    browse_button = Button(ax=ax_browse, label='Browse...', color='lightblue', hovercolor='skyblue')
    browse_button.on_clicked(browse_file)
    # Create a file display button
    file_button = Button(ax=ax_file, label=f'Current: {os.path.basename(current_file)}', color='lightgoldenrodyellow')

    # Frequency slider
    freq_slider = Slider(ax=ax_freq, label='Frequency (Hz)', valmin=freqs.min(), valmax=freqs.max(), valinit=initial_freq, 
                         valstep=None)  # Implementing continuous triggering
    # r0_rec slider (range from r0 to 2.0, step size: 0.1)
    r0_slider = Slider(ax=ax_r0, label='Reconstruction Radius (r0_rec)', valmin=r0, valmax=2.0, valinit=max(r0, initial_r0_rec), valstep=0.1)
    ...
    def update(val):
        ...
        # Plot reconstructed
        plot_balloon_rec(ax_recon, sh_order, Pnm_rec[0], dirs, az_m, el_m, 
                        f"Reconstructed {int(actual_freq)} Hz, r0_rec={r0_rec:.1f}", Ynm_cache)
...
def plot_balloon_rec(ax, order, Pnm, dirs, az_m, el_m, title, Ynm_cache):
    ...
    # Magnitude
    D_abs = np.abs(D) 
    D_plot = D_abs.reshape(el_m.shape) # (elevation, azimuth), D_abs is one-dimensional, reshape to 2D (res, 2*res+1)
    ...
    for vec, label in zip(
        ...
        ax.add_artist(arrow)
        ax.text(vec[0] * 1.05, vec[1] * 1.05, vec[2] * 1.05, label, color='black', fontsize=12)
...
if __name__ == "__main__":  
    balloon_plot_with_slider(
        data_dir = '.\data\sampled_directivity\source',  # According to current working directory
        sh_order=6,
        initial_freq=500,
        initial_r0_rec=0.6
    )