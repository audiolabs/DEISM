...
def balloon_plot_with_slider(data_dir, sh_order=4, initial_freq=500, initial_r0_rec=0.5):
    ...
    freq_slider = Slider(
        ...
        valstep=None  # Make Slider into continuous sliding mode
    )
    ...
    def load_file(file_idx):
        nonlocal current_file, current_file_idx, r0, freqs, params, Psh, Dir_all
        current_file_idx = file_idx
        current_file = os.path.join(data_dir, source_files[file_idx])
        mat = loadmat(current_file)
        
        # Read the contents from a .mat file into a Python dictionary format
        Psh = mat['Psh']
        Dir_all = mat['Dir_all']
        freqs = mat['freqs_mesh'].squeeze()
        r0 = float(mat['r0'].squeeze())
        ...
        # Update r0 slider
        r0_slider.valmin = r0  # Ensure r0_rec >= original r0
    ...
    def update(val):
        ...
        # Get SH coefficients for current frequency
        k = params["waveNumbers"][freq_idx]
        full_Pnm = SHCs_from_pressure_LS(Psh[freq_idx].reshape(1, -1), Dir_all, sh_order, np.array([actual_freq]))
        Cnm_s = get_directivity_coefs(k, sh_order, full_Pnm, r0)
        ...
        for n in range(sh_order + 1):
            hn_r0_rec = sphankel2(n, k * r0_rec)
...
def plot_balloon_rec(ax, order, Pnm, dirs, az_m, el_m, title):
    ...
    for n in range(order + 1):
        for m in range(-n, n + 1):
            Ynm = sph_harm(m, n, dirs[:, 0], dirs[:, 1])
...
if __name__ == "__main__":
    balloon_plot_with_slider(
        data_dir=r"D:\Conda\DEISM-main\DEISM-main\examples\data\sampled_directivity\source",
        sh_order=6,
        initial_freq=500,
        initial_r0_rec=0.6
    )