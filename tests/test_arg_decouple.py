...
def _cpp_trace_mask(params, room):
    ...
    # Production code drops the order-zero direct path when requested ...
    mask = np.ones(orders.shape[0], dtype=bool)
    if params["ifRemoveDirectPath"]:
        mask = orders != 0
    # ... and, in RIR mode, every image whose path exceeds c * rirPeriod
    # (it would fold into the periodic inverse FFT); see get_ref_geometry_ARG.
    if params.get("mode") == "RIR":
        from deism.core_deism import image_time_limit
        from deism.core_deism_arg import get_R_sI_to_r_from_room

        sources = np.asarray(engine.sources, dtype=np.float32)
        distances = get_R_sI_to_r_from_room(np.asarray(params["posReceiver"]), sources)[2]
        mask &= distances.astype(np.float32) <= params["soundSpeed"] * image_time_limit(params)
    return mask
