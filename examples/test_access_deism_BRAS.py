...
def main():
    ...
    plt.plot(
        t_axis[:min_len] * 1000,
        sim_norm[:min_len],
        label="Simulated RIR (DEISM 1st Order)",
        color="red",
        alpha=0.8,
        linewidth=1.2,
    )
    ...
    time_reflection = (
        (
            np.linalg.norm(source - reflection_point)
            + np.linalg.norm(receiver - reflection_point)
        )
        / 343
        * 1000
    )
    plt.axvline(
        ...
        color="blue",
        linestyle="--",
        label=f"Theoretical Direct Time {time_direct:.2f} ms",
    )
    plt.axvline(
        x=time_reflection,
        color="green",
        linestyle="--",
        label=f"Theoretical Reflection Time {time_reflection:.2f} ms",
    )
