...
All repository examples live in ``examples/``. They are not all at the same
level of maturity: some scripts use the current class-based workflow, while a
few older research scripts still use lower-level helper paths.

Recommended starting points
---------------------------

Use these first if you are learning the current API:
...
.. list-table::
   ...
   * - ``deism_singleparam_example.py``
     - shoebox
     - RIR
     - Current class-based single-run example
   * - ``deism_arg_singleparam_example.py``
     - convex
     - RIR
     - Current class-based convex-room example
   * - ``deisms_lc_mix_test.py``
     - shoebox
     - RTF
     - Compare ``ORG``, ``LC``, and ``MIX`` using the current class workflow
   * - ``shoebox_images_cal_compare.py``
     - shoebox
     - RTF
     - Compare shoebox image-calculation variants in the current class workflow
...
- Shows the current convex-room update order:
  room -> materials -> frequencies -> source/receiver -> directivities -> run.
...
- Useful when evaluating method trade-offs rather than learning the API from
  scratch.
...
The following scripts are still useful, but they are not the best introduction
to the current public workflow:
